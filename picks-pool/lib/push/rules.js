...
export function leaders(games, entries, picks, scoring = 'straight', lock = false) {
  const { rows, finals } = slateResults(games, entries, picks, { scoring, lock });
  if (!finals || !rows.length || rows[0].points === 0) return { key: '', ids: [], rows };
